...
#endif
#include <iostream>
class NFALU_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    NFALU_api_t(VNFALU* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_rmm)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_aluCtl)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_input2)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_input1)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_result)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["NFALU.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VNFALU* top = new VNFALU;
    std::string vcdfile = "test_run_dir/NFALU_Test/NFALU.vcd";
    ...
    Verilated::mkdir("test_run_dir/NFALU_Test/logs");
    VerilatedCov::write("test_run_dir/NFALU_Test/logs/coverage.dat");
#endif
    delete top;
    exit(0);
}
